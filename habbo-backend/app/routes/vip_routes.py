...
VIP_RANK_META = {
    11: {"name": "VIP Diamond", "icon": "diamond", "color": "from-cyan-500 to-blue-500", "badge_color": "bg-cyan-900/50 text-cyan-300 border-cyan-700/50"},
    10: {"name": "VIP Gold", "icon": "crown", "color": "from-yellow-600 to-amber-500", "badge_color": "bg-yellow-900/50 text-yellow-300 border-yellow-700/50"},
    9: {"name": "VIP Silver", "icon": "gem", "color": "from-zinc-500 to-zinc-400", "badge_color": "bg-zinc-700/50 text-zinc-300 border-zinc-500/50"},
    8: {"name": "VIP Bronze", "icon": "star", "color": "from-orange-700 to-orange-500", "badge_color": "bg-orange-900/50 text-orange-300 border-orange-700/50"},
}
...
async def get_vip_list():
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            try:
                await cur.execute("SELECT id, rank_name FROM permissions WHERE id >= 8 AND id <= 11 ORDER BY id DESC")
            ...
            # Get all VIP members (ranks 8-11)
            await cur.execute(
                "SELECT id, username, look, motto, `rank`, online FROM users WHERE `rank` >= 8 AND `rank` <= 11 ORDER BY `rank` DESC, username ASC"
            )
    ...
    # Ensure all VIP ranks 8-11 are shown even if empty
    for r in range(11, 7, -1):
